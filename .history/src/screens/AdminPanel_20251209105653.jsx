...
const AdminPanel = ({ t, status, formData, setStatus, setScreen }) => {
  ...
  // Helper to show uploaded document preview
  const renderImageBox = (label, fileData) => {
    return (
      <div className="bg-gray-100 rounded-lg h-24 flex items-center justify-center overflow-hidden">
        {fileData && fileData.files && fileData.files.length > 0 ? (
          <img 
            src={fileData.files[0].preview || fileData.files[0].url} 
        ...
        {status === 'submitted' ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="p-4 grid grid-cols-2 gap-2">
              ...
              {/* ID FRONT */}
              {renderImageBox("ID Front", formData.docFront)}

              {/* ID BACK */}
              {renderImageBox("ID Back", formData.docBack)}
