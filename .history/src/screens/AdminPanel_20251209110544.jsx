...
const AdminPanel = ({ t, status, formData, setStatus, setScreen }) => {
  ...
  const renderPreview = (fileObj) => {
    ...
    // If PDF → show name only
    if (file.type.includes("pdf")) {
      return (
        <div className="text-xs text-gray-600 text-center">
          <span className="font-bold">PDF:</span><br />
          {file.name}
        </div>
      );
    }
  ...
  // Selfie preview
  const renderSelfie = () => {
    if (!formData.selfie || !formData.selfie.previewURL) {
      return <span className="text-gray-400 text-xs">Selfie Not Captured</span>;
    }
    return (
      <img 
        src={formData.selfie.previewURL}
        alt="Selfie"
        className="w-full h-full object-cover rounded-lg"
      />
    );
        ...
        {status === 'submitted' ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="p-4 border-b border-gray-100 flex justify-between items-start">
              <div>
                <h4 className="font-bold text-gray-800">{formData.name || "User"}</h4>
            ...
            <div className="p-4 grid grid-cols-2 gap-2">
              ...
              {/* ID FRONT */}
              <div className="bg-gray-100 rounded-lg h-28 overflow-hidden flex items-center justify-center">
              ...
              {/* SELFIE */}
              <div className="bg-gray-100 rounded-lg h-32 col-span-2 overflow-hidden flex items-center justify-center">
                {renderSelfie()}
              </div>
