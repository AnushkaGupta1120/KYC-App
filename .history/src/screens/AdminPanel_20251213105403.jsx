...
export default function AdminPanel({
  ...
  useEffect(() => {
    console.log("AdminPanel status:", status);
  }, [status]);
  ...
  return (
    <div className="p-6">
      ...
      {/* ✅ EMPTY STATE — NO KYC YET */}
      {!isKYCSubmitted || !formData ? (
        ...
        <>
          ...
          <div className="bg-white p-5 rounded-xl shadow mb-6 border">
            ...
            <p><strong>Name:</strong> {renderValue(formData.name)}</p>

            <p>
              <strong>ID Type:</strong>{" "}
              {formData.idType === "aadhaar"
                ? "Aadhaar Card"
                : renderValue(formData.idType)}
            </p>

            <p>
              <strong>ID Number:</strong>{" "}
              {renderValue(formData.idNumber)}
            </p>
