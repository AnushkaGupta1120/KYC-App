           ...
           <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
             ...
             >
                {formData.docFront ? (
                  <>
                    ...
                    <span className="text-sm font-bold text-green-700">✓ {frontFileInfo?.count || 1} file(s) uploaded</span>
                    <span className="text-xs text-gray-600 text-center break-words max-w-xs">{frontFileInfo?.names || formData.docFront}</span>
                    {frontFileInfo?.sizes && <span className="text-xs text-gray-500">{frontFileInfo.sizes}</span>}
                  </>
                ) : (
                  <>
                    ...
                    <span className="text-xs text-gray-400">Drag & drop or click to select files</span>
             ...
             >
                {formData.docBack ? (
                  <>
                    <CheckCircle className="text-green-600" size={32} />
                    <span className="text-sm font-bold text-green-700">✓ {backFileInfo?.count || 1} file(s) uploaded</span>
                    <span className="text-xs text-gray-600 text-center break-words max-w-xs">{backFileInfo?.names || formData.docBack}</span>
                    {backFileInfo?.sizes && <span className="text-xs text-gray-500">{backFileInfo.sizes}</span>}
                  </>
                ) : (
                  <>
                    <div className="w-12 h-12 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-600"><Upload size={20} /></div>
