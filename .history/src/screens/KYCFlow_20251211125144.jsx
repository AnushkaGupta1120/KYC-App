...
export default function KYCFlow({
  ...
  const goNext = () => {
    if (kycStep < 4) {
      speak(t.voice_next_step);
      setKycStep(kycStep + 1);
      setTimeout(() => {
        speak(
          kycStep + 1 === 2 ? t.voice_step2 :
          kycStep + 1 === 3 ? t.voice_step3 :
          kycStep + 1 === 4 ? t.voice_step4 :
          t.voice_step1
        );
      }, 500);
    }
  ...
  const goBack = () => {
    if (kycStep > 1) {
      speak(t.voice_previous_step);
      setKycStep(kycStep - 1);

      setTimeout(() => {
        speak(
          kycStep - 1 === 1 ? t.voice_step1 :
          kycStep - 1 === 2 ? t.voice_step2 :
          kycStep - 1 === 3 ? t.voice_step3 :
          t.voice_step4
        );
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="p-4 bg-white border-t">
        {kycStep < 4 ? (
          <button onClick={goNext} className="w-full bg-indigo-600 text-white py-3 rounded-xl">
            {t.next}
          </button>
        ) : (
          <button onClick={submitKYC} className="w-full bg-green-600 text-white py-3 rounded-xl">
            {t.submit_btn}
          </button>
        )}
      </div>
